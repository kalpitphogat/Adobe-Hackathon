...
def _strip_tool_args(tool: str) -> str:
    """`Bash(python:*)` -> `Bash`, so a space inside an argument is not a split."""
    return re.sub(r"\(.*?\)", "", tool).strip()
...
def check_skill_md(res: Results, skills: list[dict]) -> None:
    ...
    for skill in skills:
        ...
        # The handout asks each skill to declare its tool needs. agentskills.io
        # writes allowed-tools as a comma-separated list; a space-separated one
        # parses as a single tool named "Bash(python:*) Read ..." and silently
        # grants nothing.
        tools = fm.get("allowed-tools")
        if not tools:
            res.fail(f"skill.{label}.allowed_tools", "missing; the handout asks skills to declare tool needs")
        else:
            entries = [t.strip() for t in tools.split(",")]
            if any(not t for t in entries):
                res.fail(f"skill.{label}.allowed_tools_format", f"empty entry in {tools!r}")
            elif any(" " in _strip_tool_args(t) for t in entries):
                res.fail(
                    f"skill.{label}.allowed_tools_format",
                    f"{tools!r} is not comma-separated; a space-separated list parses as one tool",
                )
            else:
                res.ok(f"skill.{label}.allowed_tools", ", ".join(entries))
...
def check_readme_counts(res: Results, totals: dict | None) -> None:
    ...
    check_prose_counts(res, totals, text, m.span())


COUNT_OF_TOTAL_RE = re.compile(r"\b(\d+)\s+of\s+(\d+)\s+checks\b")
BARE_COUNT_RE = re.compile(r"\b(\d+)\s+checks\b")


def check_prose_counts(res: Results, totals: dict, text: str, auto_span: tuple[int, int]) -> None:
    """F1, the half the INVENTORY block does not cover.

    The generated block keeps itself honest, but a count written by hand
    somewhere else in the prose drifts silently -- which is exactly how
    "61 of 64 checks" outlived a registry that had moved to 65. Every count
    outside the block is therefore checked against the live registry too.
    """
    total = totals["core"] + totals["enrichment"]
    prose = text[:auto_span[0]] + text[auto_span[1]:]

    # "N of M checks": M is always the total; N is one of the tiers.
    bad = [f"{a} of {b}" for a, b in
           (m.groups() for m in COUNT_OF_TOTAL_RE.finditer(prose))
           if int(b) != total or int(a) not in (totals["core"], totals["enrichment"])]
    if bad:
        res.fail("readme.prose_counts",
                 f"{bad} disagrees with the registry "
                 f"({totals['core']} core / {totals['enrichment']} enrichment / {total} total)")
        return

    # A bare "N checks" can only mean the total.
    stripped = COUNT_OF_TOTAL_RE.sub("", prose)
    wrong = [m.group(1) for m in BARE_COUNT_RE.finditer(stripped) if int(m.group(1)) != total]
    if wrong:
        res.fail("readme.prose_counts",
                 f"'{wrong[0]} checks' in prose disagrees with the registry total of {total}")
        return

    res.ok("readme.prose_counts", f"every count in prose matches the registry ({total} checks)")
