# AI-readiness audit — northwind-analytics.test

*Audited 2026-05-28T20:26:40Z · status: **complete***

*Evidence basis: **8 page(s) fetched**, 4 rendered. Every finding below applies only to the URLs it lists. Pages this crawl did not fetch were not assessed — absence of a finding for a page is not a pass for it.*

## What to do first

**F-001 · MEDIUM** — fact_coverage 8/9: 1 claim(s) cannot be quoted from this page
...
**F-002 · MEDIUM** — Nothing near the top of the page states plainly what Priya Raman is — on /blog/measuring-pipeline-latency
...
- **Effort:** low · **ICE:** 6.7
- **Verify:** `Read the first two sentences of https://northwind-analytics.test/blog/measuring-pipeline-latency: do they answer "what is this?" without further reading?`

**F-003 · MEDIUM** — Nothing near the top of the page states plainly what Northwind Analytics is — on /docs/getting-started

- **Do:** Open with one sentence of the form "Northwind Analytics is a … that …".
- **Why it works:** A short definitional sentence is directly quotable as an answer; a reader that has to synthesise one from scattered prose usually picks a source that did the work.
- **Effort:** low · **ICE:** 6.7
- **Verify:** `Read the first two sentences of https://northwind-analytics.test/docs/getting-started: do they answer "what is this?" without further reading?`

**F-004 · MEDIUM** — Nothing near the top of the page states plainly what Northwind Analytics Team is — on /pricing

- **Do:** Open with one sentence of the form "Northwind Analytics Team is a … that …".
- **Why it works:** A short definitional sentence is directly quotable as an answer; a reader that has to synthesise one from scattered prose usually picks a source that did the work.
- **Effort:** low · **ICE:** 6.7
- **Verify:** `Read the first two sentences of https://northwind-analytics.test/pricing: do they answer "what is this?" without further reading?`

**F-005 · MEDIUM** — A pricing page offers no evidence that anyone else has trusted this

- **Do:** Add one specific, attributed piece of evidence near the primary action.
- **Why it works:** An attributed third-party statement is evidence a visitor can weigh; a first-party claim is not, so it does not reduce the risk of acting.
- **Effort:** low · **ICE:** 6.7
- **Verify:** `Open https://northwind-analytics.test/pricing: is there evidence from someone other than the brand?`
...
```
Read the first two sentences of https://northwind-analytics.test/blog/measuring-pipeline-latency: do they answer "what is this?" without further reading?
```
...
```
Read the first two sentences of https://northwind-analytics.test/docs/getting-started: do they answer "what is this?" without further reading?
```
...
```
Read the first two sentences of https://northwind-analytics.test/pricing: do they answer "what is this?" without further reading?
```

*Source: references/geo-methods.md*

### F-006 · MEDIUM · A article page carries only 277 words

*confidence: confirmed · check: `extract.ans.thin_content_for_page_type`*

**Evidence.** https://northwind-analytics.test/blog/measuring-pipeline-latency has 277 words of main content against a floor of 300 for a article page on a saas-marketing site. Navigation, header and footer were excluded from the count. There is not enough here for a retrieval system to find an answer to anything specific.
...
```
curl -s https://northwind-analytics.test/blog/measuring-pipeline-latency | sed -e 's/<[^>]*>//g' | wc -w
```
...
**Evidence.** https://northwind-analytics.test/pricing (159 words) contains none of: review or rating language, a customer or user count, a case study reference, or an attributed quotation. A visitor deciding whether to act has only the brand's own claims to go on.
...
```
Open https://northwind-analytics.test/pricing: is there evidence from someone other than the brand?
```
...
*brand-ai-readiness-audit 1.0.0 · 5 of 66 checks ran · AI crawler list snapshot 2026-09-09 (commit 0e111dcc24cb) · recommend-only: nothing was written to the audited site.*
