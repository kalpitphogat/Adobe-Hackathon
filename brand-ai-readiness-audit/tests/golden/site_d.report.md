...
*Evidence basis: **4 page(s) fetched**, 0 rendered. Every finding below applies only to the URLs it lists. Pages this crawl did not fetch were not assessed — absence of a finding for a page is not a pass for it.*

## What to do first

**F-001 · HIGH** — The served HTML is an empty application shell with no content — on the homepage
...
**F-002 · HIGH** — The served HTML is an empty application shell with no content — on /about
...
**F-003 · HIGH** — The served HTML is an empty application shell with no content — on /features
...
**F-004 · HIGH** — The served HTML is an empty application shell with no content — on /pricing
...
- 4 finding(s) are recorded but capped at medium because F-001 (The served HTML is an empty application shell with no content — on the homepage) blocks them. They become relevant the moment F-001 is fixed.
- 1 finding(s) are recorded but capped at medium because F-002 (The served HTML is an empty application shell with no content — on /about) blocks them. They become relevant the moment F-002 is fixed.
- 1 finding(s) are recorded but capped at medium because F-003 (The served HTML is an empty application shell with no content — on /features) blocks them. They become relevant the moment F-003 is fixed.
- 1 finding(s) are recorded but capped at medium because F-004 (The served HTML is an empty application shell with no content — on /pricing) blocks them. They become relevant the moment F-004 is fixed.
...
- **F-005** · medium · The page title is 'Vantage', which does not say what the page is about — on the homepage — blocked by F-001 (`extract.ans.title_not_entity_bearing`)
- **F-006** · medium · The page title is 'Vantage', which does not say what the page is about — on /about — blocked by F-002 (`extract.ans.title_not_entity_bearing`)
- **F-007** · medium · The page title is 'Vantage', which does not say what the page is about — on /features — blocked by F-003 (`extract.ans.title_not_entity_bearing`)
- **F-008** · medium · The page title is 'Vantage', which does not say what the page is about — on /pricing — blocked by F-004 (`extract.ans.title_not_entity_bearing`)
...
*brand-ai-readiness-audit 1.0.0 · 6 of 66 checks ran · AI crawler list snapshot 2026-09-09 (commit 0e111dcc24cb) · recommend-only: nothing was written to the audited site.*
